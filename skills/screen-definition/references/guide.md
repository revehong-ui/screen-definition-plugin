...
- **편집**: 연필 아이콘 → 인라인 편집 폼
- **삭제**: 휴지통 아이콘 → 즉시 삭제 (히스토리에 기록, Undo 가능, 새로고침 후에도 삭제 유지)
- **순서 조정**: 위/아래 화살표 버튼(▲▼)으로 서브아이템 순서 변경 (첫 번째/마지막 항목은 해당 방향 비활성화)
...
- **삭제된 서브아이템은 `deletedSubItemIds`로 추적**되어 새로고침/마이그레이션 시에도 복원되지 않습니다
- Undo로 섹션을 복원하면 해당 서브아이템도 `deletedSubItemIds`에서 자동으로 제거됩니다
- **존재하지 않는 CSS 셀렉터**를 가진 서브아이템은 로드 시 자동으로 셀렉터가 초기화됩니다
